...
function drawBg() {
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (dist < 130) {
        ...
        bgCtx.strokeStyle = `rgba(201,146,10,${0.12 * (1 - dist / 130)})`;
        bgCtx.lineWidth   = 0.5;
        bgCtx.stroke();
      }
    }
  ...
  nodes.forEach((n, i) => {
    ...
    bgCtx.fillStyle = i % 3 === 0 ? 'rgba(155,111,224,0.45)' : 'rgba(201,146,10,0.5)';
    bgCtx.fill();
  });
...
if (helixCanvas) {
  ...
  function drawHelix() {
    ...
    for (let strand = 0; strand < 2; strand++) {
      ...
      hCtx.strokeStyle  = strand === 0 ? '#c9920a' : 'rgba(155,111,224,0.8)';
      hCtx.lineWidth    = 2;
      hCtx.shadowColor  = strand === 0 ? '#c9920a' : '#9b6fe0';
      hCtx.shadowBlur   = 8;
      hCtx.stroke();
      hCtx.shadowBlur   = 0;
    }
    ...
    for (let i = 0; i <= PTS; i += 4) {
      ...
      hCtx.strokeStyle = `rgba(201,146,10,${alpha})`;
      hCtx.lineWidth   = 1;
      hCtx.stroke();

      // node dots
      [[x1, '#f0b429'], [x2, '#9b6fe0']].forEach(([x, color]) => {
        hCtx.beginPath();
